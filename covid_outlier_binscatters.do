...
gen rvk_oecd_ex_big = rvk_oecd & country != "United States" & country != "Germany" & country != "Japan" & country != "France" & country != "Netherlands"
...
local struct_order_str "gRGDP news_1yrs_ago news_2yrs_ago news_diff_1yrs_ago news_diff_2yrs_ago f_news_1yrs_ago f2_news_2yrs_ago f_news_diff_1yrs_ago f2_news_diff_2yrs_ago d_tot l_`depvar'"

local fs_order_str "shock_t shock_fct1 shock_fct2 d_shock_fct1 d_shock_fct2 f_shock_fct1 f2_shock_fct2 f_d_shock_fct1 f2_d_shock_fct2 d_tot l_`depvar'"
...
* Define recession years as having >= 3 months in recession
*gen rec_ind = (year == 1990 | year == 1991 | year == 2001 | year == 2008 | year == 2009)
gen rec_ind = year >= 2008 & year <= 2012

* separate category for covid
replace rec_ind = 2 if year == 2020

winsor2 *`depvar' *news_* gRGDP *tot *shock_* if time_in_sample, replace c(1 99)
winsor2 *`depvar' *news_* gRGDP *tot *shock_* if year == 2020, replace c(1 99)

*local ylims `" "-8 10" "-4 4" "-6 5" "'
local j = 1

foreach i of local cats {

binscatter `depvar' f_news_1yrs_ago if `i', controls(l_`depvar' pctd_tot gRGDP) by(rec_ind) absorb(countryid) savegraph(`outdir'/covid_binscatter_f_lvl_`i'.png) legend(order(1 "All other years" 2 "Great Recession, 2008-2012" 3 "Covid recession, 2020")) xtitle("1-Year forward output growth forecast (percentage points)") ytitle("Primary balance (percentage points)") replace //ysc(r(-8 5)) xsc(r(-4 10))
binscatter `depvar' f_news_diff_1yrs_ago if `i', controls(l_`depvar' pctd_tot gRGDP) by(rec_ind) absorb(countryid) savegraph(`outdir'/covid_binscatter_f_diff_`i'.png) legend(order(1 "All other years" 2 "Great Recession, 2008-2012" 3 "Covid recession, 2020")) xtitle("1-Year forward differenced output growth forecast (percentage points)") ytitle("Primary balance (percentage points)") replace //ysc(r(-8 5)) xsc(r(-4 10))

binscatter `depvar' news_1yrs_ago if `i', controls(l_`depvar' pctd_tot gRGDP) by(rec_ind) absorb(countryid) savegraph(`outdir'/covid_binscatter_lvl_`i'.png) legend(order(1 "All other years" 2 "Great Recession, 2008-2012" 3 "Covid Recession, 2020")) xtitle("1-Year forward output growth forecast (percentage points)") ytitle("Primary balance (percentage points)") replace //ysc(r(-8 5)) xsc(r(-4 10))
binscatter `depvar' news_diff_1yrs_ago if `i', controls(l_`depvar' pctd_tot gRGDP) by(rec_ind) absorb(countryid) savegraph(`outdir'/covid_binscatter_diff_`i'.png) legend(order(1 "All other years" 2 "Great Recession, 2008-2012" 3 "Covid Recession, 2020")) xtitle("1-Year forward differenced output growth forecast (percentage points)") ytitle("Primary balance (percentage points)") replace //ysc(r(-8 5)) xsc(r(-4 10))
