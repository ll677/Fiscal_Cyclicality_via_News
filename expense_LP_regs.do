...
gen wb_wdi_primgexp = wb_wdi_expense - wb_wdi_gov_intexp + wb_wdi_gov_net_inv

sysdir set PLUS ado
local minhz = 0
local maxhz = 5
local depvar wb_wdi_primgexp
local outdir $rootdir/tables/expense/LP
...
local order_str "gRGDP news_1yrs_ago news_diff_1yrs_ago"
 
local endog_sets `" "news_1yrs_ago" "news_diff_1yrs_ago" "news_1yrs_ago news_2yrs_ago" "news_diff_1yrs_ago news_diff_2yrs_ago" "f_news_1yrs_ago" "f_news_diff_1yrs_ago" "f_news_1yrs_ago f2_news_2yrs_ago" "f_news_diff_1yrs_ago f2_news_diff_2yrs_ago" "'

local iv_sets `" "shock_fct1" "d_shock_fct1" "shock_fct1 shock_fct2" "d_shock_fct1 d_shock_fct2" "f_shock_fct1" "f_d_shock_fct1" "f_shock_fct1 f2_shock_fct2" "f_d_shock_fct1 f2_d_shock_fct2" "'
...
local fs_order_str "gRGDP shock_fct1 shock_fct2 d_shock_fct1 d_shock_fct2 d_shock_fctF2 f_shock_fct1 f2_shock_fct2 f_d_shock_fct1 f2_d_shock_fct2 pctd_tot l_wb_wdi_debt l_`depvar'"
...
winsor2 *`depvar' *news_* gRGDP *tot *shock_* l_wb_wdi_debt if time_in_sample, replace c(1 99)
...
foreach i of local cats {
	...
	* OLS
	qui: xtreg `depvar' gRGDP `endogs' pctd_tot l_wb_wdi_debt l_`depvar' if `i' & time_in_sample , fe cluster(countryid)
	qui: est store ols_`i'
	qui: estadd ysumm
	qui: est save results/ols_`i'_`filename_spec', replace
	* Reduced form
	qui: xtreg `depvar' gRGDP `ivs' pctd_tot l_wb_wdi_debt l_`depvar' if `i' & time_in_sample , fe cluster(countryid)
	qui: est store rf_`i'
	qui: estadd ysumm
	qui: test `ivs'
	...
	* First stages
	...
	foreach m of num 1/`num_endogs' {
		...
		qui: xtreg `m_endog' gRGDP `ivs' pctd_tot l_wb_wdi_debt l_`depvar' if `i' & time_in_sample , fe cluster(countryid)
	...
	* IV
	qui: xi: xtivreg2 `depvar' (`endogs' = `ivs') gRGDP pctd_tot l_wb_wdi_debt l_`depvar' if `i' & time_in_sample, fe ffirst cluster(countryid)
	qui: est store iv_`i'
	qui: estadd ysumm
...
esttab `olsregs' ///
		...
		labels("Mean Y" "Observations")) mtitle(`"`cat_labels'"') order(`struct_order_str') label substitute(\_ _)
esttab `rfregs' ///
		using "`outdir'/by_regtype/rf__`filename_spec'.tex", replace booktabs b(3) se(3) ///
		star(* 0.10 ** 0.05 *** 0.01) nonotes stats(ymean N shock_F, fmt(2 0 3) ///
		labels("Mean Y" "Observations" "Exc. Inst. F-stat")) mtitle(`"`cat_labels'"') order(`struct_order_str') label substitute(\_ _)
esttab `ivregs' ///
...
foreach i of local cats {
	...
	esttab `olsregs' ///
		...
		star(* 0.10 ** 0.05 *** 0.01) nonotes stats(ymean N, fmt(2 0) ///
		labels("Mean Y" "Observations")) order(`struct_order_str') label substitute(\_ _)
	esttab `rfregs' ///
		using "`outdir'/by_group/rf_`i'_regs.tex", replace booktabs b(3) se(3) ///
		star(* 0.10 ** 0.05 *** 0.01) nonotes stats(ymean N shock_F, fmt(2 0 3) ///
		labels("Mean Y" "Observations" "Exc. Inst. F-stat")) order(`struct_order_str') label substitute(\_ _)
	esttab `ivregs' ///
