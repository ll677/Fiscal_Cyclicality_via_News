...
local outdir $rootdir/tables/main/contemp_only
...
winsor2 `depvar' news_* gRGDP tot shock_*, replace c(1 99)
...
local endog_sets `" "gRGDP" "'

local iv_sets `" "shock_t" "'

local struct_order_str "gRGDP  d_tot l_`depvar'"

local fs_order_str "shock_t d_tot l_`depvar'"
...
la var d_tot "Differenced terms-of-trade index (\%)"
...
foreach i of local cats {
	...
	* OLS
	qui: xtreg `depvar' `endogs' d_tot l_`depvar' if `i' == 1, fe cluster(countryid)
	qui: est store ols_`i'
	qui: estadd ysumm
	qui: est save results/ols_`i'_`filename_spec', replace
	* Reduced form
	qui: xtreg `depvar' `ivs' d_tot l_`depvar' if `i' == 1, fe cluster(countryid)
	qui: est store rf_`i'
	qui: estadd ysumm
	qui: test `ivs'
	...
	* First stages
	...
	foreach m of num 1/`num_endogs' {
		...
		qui: xtreg `m_endog' `ivs' d_tot l_`depvar' if `i' == 1, fe cluster(countryid)
	...
	* IV
	qui: xi: xtivreg2 `depvar' (`endogs' = `ivs') d_tot l_`depvar' if `i' == 1, fe ffirst cluster(countryid)
	qui: est store iv_`i'
	qui: estadd ysumm
