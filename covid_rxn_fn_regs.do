...
local outdir $rootdir/tables/covid/rxn_fn
...
foreach i of local cats {
	...
	* OLS
	qui: xtreg `depvar' `endogs' pctd_tot l_`depvar' if `i' & time_in_sample, fe cluster(countryid)
	qui: est store ols_`i'
	qui: estadd ysumm
	qui: est save results/ols_`i'_`filename_spec', replace
	* Reduced form
	qui: xtreg `depvar' `ivs' pctd_tot l_`depvar' if `i' & time_in_sample, fe cluster(countryid)
	qui: est store rf_`i'
	qui: estadd ysumm
	qui: test `ivs'
	...
	* First stages
	...
	foreach m of num 1/`num_endogs' {
		...
		qui: xtreg `m_endog' `ivs' pctd_tot l_`depvar' if `i' & time_in_sample, fe cluster(countryid)
	...
	* IV
	qui: xi: xtivreg2 `depvar' (`endogs' = `ivs') pctd_tot l_`depvar' if `i' & time_in_sample, fe ffirst cluster(countryid)
	qui: est store iv_`i'
	qui: estadd ysumm
