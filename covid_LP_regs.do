...
gen time_in_sample = year >= 1990 & year <= 2020

sysdir set PLUS ado
local minhz = 0
local maxhz = 5
local depvar wb_wdi_primbal
local outdir $rootdir/tables/covid/LP
...
local fs_order_str "gRGDP shock_fct1 shock_fct2 d_shock_fct1 d_shock_fct2 d_shock_fctF2 f_shock_fct1 f2_shock_fct2 f_d_shock_fct1 f2_d_shock_fct2 pctd_tot l_`depvar'"
...
foreach i of local cats {
	...
	* OLS
	qui: xtreg `depvar' gRGDP `endogs' pctd_tot l_`depvar' if `i' & time_in_sample , fe cluster(countryid)
	qui: est store ols_`i'
	qui: estadd ysumm
	qui: est save results/ols_`i'_`filename_spec', replace
	* Reduced form
	qui: xtreg `depvar' gRGDP `ivs' pctd_tot l_`depvar' if `i' & time_in_sample , fe cluster(countryid)
	qui: est store rf_`i'
	qui: estadd ysumm
	qui: test `ivs'
	...
	* First stages
	...
	foreach m of num 1/`num_endogs' {
		...
		qui: xtreg `m_endog' gRGDP `ivs' pctd_tot l_`depvar' if `i' & time_in_sample , fe cluster(countryid)
	...
	* IV
	qui: xi: xtivreg2 `depvar' (`endogs' = `ivs') gRGDP pctd_tot l_`depvar' if `i' & time_in_sample, fe ffirst cluster(countryid)
	qui: est store iv_`i'
	qui: estadd ysumm
